...
HELD = [
    ...
    },
    {
        ...
        'sector': '面板/族群補漲',
        'note': '成本算分倉 $58.7 (user 2026-06-28 拍板: 剩2張=原始低成本倉、買回churn的虧損當作已實現先出)。⚠️券商庫存用加權平均顯示 ~$69/-5.74%、跟這裡分倉視角差 ~$20k = 加權平均vs FIFO會計差、非實際虧。分倉視角 = 群創現$65仍 +11% 獲利倉 → 拉高清掉是鎖利非認賠。⚠️但外資6/26巨倒-152k派發、雞肋(老師6/28)、拉高反彈清2張、別殺低、不加'
    },
    ...
    {
        'ticker': '2303', 'name': '聯電',
        'cost': 158.0, 'shares': 4000, 'stop': 158.7,
        'strategy_mode': 'swing',
        'tactic': '戰略級', 'priority': 3,
        'source': '老師明示 6/14 戰略級 + 黃大大波段(65%權重) + 6/16「黑K+籌碼漂亮」',
        'sector': '成熟製程',
        'note': '6/17 $139 + 6/18 $144 + 6/23 $171 + 6/25 $178(黃大大波段加碼) = 4張 avg $158.0。⚠️6/25 $178 加碼依據黃大消息(65%權重)+外資6/24還買+7,333(到貨沒成真);但距五日線+13.4%=老師「離五日線就危險」zone。stop→守五日線$158.7收盤(老師6/24 anchor、收破=離五日線太遠+到貨確認→出)。注意: $178那張回五日線是-11%、是追高代價;3張便宜倉仍大賺。🔴不再加(已4張、距五日線遠)'
    },
    {
    ...
    },
    {
        ...
        'source': '自選(無thesis)',
        'sector': '電源/重電',
        'note': '6/24 買 100股 @$2030。6/25 尾盤賣 50股 @$2000 (實現 -$1.5k、原以為只有50股、其實買100)。剩 50股 @$2030。⚠️最弱一筆: 空頭排列(MA5<MA10<MA20)、外資賣、老師沒提 = 接刀無thesis。stop $1970(收盤跌破才出 / 殺盤日開盤5分內出)。剩50股小錢、本來就想清掉、可考慮剩的也出乾淨、不加碼'
    },
]
