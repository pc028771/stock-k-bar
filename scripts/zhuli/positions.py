...
HELD = [
    ...
    },
    {
        ...
        'sector': '面板/族群補漲',
        'note': '🔒 6/4 進處置→6/18 出關。6/17 漲停 +9.94%。6/22 加碼 +2張 @$68.7 (黃大、均 $63.7、共 4張)。📌 6/23 user 拍板: 消息面持有(黃大+未來展望)、不主動減、守 stop。⚠️ 20d 外資 -382k 派發 + 距MA10 +24% 末端 (籌碼軸唯一壞的)、但消息面 override → 不加碼亦不主動砍、stop $58.5 (6/18 結構低、收盤跌破才出)'
    },
    # 8046 南電 6/5 全清 (1000 @ $851 + 200 @ $838、共鎖 ~$57.7k 損)
    {
        ...
        'sector': 'ABF/塑化',
        'note': '6/5 Stage 1 @ $104.5、6/17 漲停 +10% @$126.5 確認籌碼勝拉高出貨、stop $96.9→$115 (6/16 收盤鎖利)、浮盈 +21%、Stage 2 加碼需等回測 MA5/MA10、不漲停加'
    },
    # 3264 欣銓 6/10 全清 @ $222 (-$7,345 / -3.21%)、按尼克老師「沒收復 6/5 低」規則出場
    # 6257 矽格 6/10 全清 @ $212 (-$11,808 / -5.30%)、同 3264 一起減倉
    {
        'ticker': '6239', 'name': '力成',
        'cost': 346.0, 'shares': 1000, 'stop': 337.5,
        'strategy_mode': 'swing',
        'tactic': '題材', 'priority': 3,
        'source': '老師三重背書 6/13 OSAT + 6/14 融資股 + 6/11 直播',
        'sector': '封測 OSAT',
        'note': '6/16 Stage 1 進 1 張 @ $346。6/22 stop trail → $337.5 (6/18 結構低、+9.90%突破K的底)、收盤跌破才出。原 $340 掛在結構低之上=會被正常回測洗掉(premature)。現 $370 +7%、距MA10 +11% 健康續攻、未到+10%不加碼'
    },
    {
        'ticker': '2303', 'name': '聯電',
