...
PLAN_PRIMARY: list = [
    ...
    #
    # 6/17 2303 聯電新進場 (老師「黑K + 籌碼漂亮 → 隔天可操作」模式、3008 大立光範例對照)
    {
        'ticker': '2303', 'name': '聯電',
        'shares': 1000,                  # 1 張試水 (Stage 1)
        'target_price': 141.5,           # 6/16 close
        'stop': 138.0,                   # 6/16 L 結構底
        'condition': (
            '🔴 強訊號日紀律例外 (per user 6/17 challenge): '
            '9:30 後 (前 30 分鐘觀察過) + 守 MA10 ($129.6) + '
            '不跳空 +3%+ + 漲幅 < +5% (不大紅) → 可 Stage 1 試水 1 張 (雙錨停損)。'
            '13:00 後尾盤仍是首選窗口、但 9:30 後可進。'
            '⚠️ 不開盤 (9:00 試撮) 進、避免結算日洗盤'
        ),
        'priority': 'new_entry',
        'rationale': (
            '完美對應老師「黑K + 籌碼漂亮 → 隔天可操作」模式 (3008 大立光 6/12→6/15-16 範例、'
            '其中 3008 6/15 10:01 鎖漲停 → 強訊號日尾盤進不到): '
            '6/16 綠K body + 外資 +28,998 史詩級 + 投信 +11,348 大買、'
            '距 MA10 +8.9% 打擊區邊緣、戰略級 ⭐⭐⭐ 4 大論述、CB 7/中觀察、'
            '6/15 處置出關後首次回測。2303 是大型權值股流通大、鎖漲停機率 < 10%、'
            '但仍按「強訊號日 9:30 後可進」紀律例外處理。'
        ),
        'sizing': '$141k (~4.4% of cash)、保守 1 張試水',
        'skip_if': '9:00 試撮跳空 ≥ +3% / 9:30 後仍跌破 MA10 / 大紅 +5%+ / 籌碼次日轉空'
    },
