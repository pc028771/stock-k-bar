...
HELD = [
    ...
    {
        'ticker': '2303', 'name': '聯電',
        'cost': 174.0, 'shares': 2000, 'stop': 148.0,
        'strategy_mode': 'swing',
        'tactic': '戰略級', 'priority': 3,
        'source': '老師明示 6/14 戰略級 + 黃大大波段(65%權重) + 6/16「黑K+籌碼漂亮」',
        'sector': '成熟製程',
        'note': '成本$174 FIFO(剩2張、原4張減半)。進處置7/2-7/15。🔴7/2邊緣收破10日線($165.5<$165.9差$0.4)+外資-12,538投信-17,511大賣(處置日部分機械性)。**user 7/2知情續抱**(避免踏空+處置難再進、賭N字/CB catalyst、老師7月關注聯電)。線改「守月線MA20 $148」(收破才出、10日線只是短線操作線、抱波段賭catalyst守結構底)。每天EOD盯、破$148或籌碼持續非機械性惡化→提醒user。加碼等出關7/16後回測 or CB(7月中定價)、別追、別加碼在處置期'
    },
    {
