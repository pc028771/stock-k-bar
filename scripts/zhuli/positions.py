...
HELD = [
    # 6285 啟碁 6/15 全清 @ $273 (-$43,053 / -13.66%)、按紀律出場 (晚 5 天、6/8 已破停損)
    {
        ...
        'source': '老師重壓 + 元大館前哥(tier-1分點) override',
        'sector': '紅海第二棒',
        'note': '6/2 8張@$40.1 + 6/3 加4張@$40.5、均$40.23。6/23 收$37.80 破結構$38.1 + 外資-21,476大賣、但元大館前哥(tier-1)連5盤累積+13k張、6/23 -7%黑K還買+1,118 = 主力收外資倒貨 → 館前哥 override 抱。stop $38.75→$37.65(今日低、收盤破=連分點成本帶都守不住就認輸)。⚠️不加碼(已core重壓)、6/24必看館前哥續買否、轉賣立刻砍'
    },
    {
    ...
    {
        'ticker': '2303', 'name': '聯電',
        'cost': 141.5, 'shares': 2000, 'stop': 160.0,
        'strategy_mode': 'swing',
        'tactic': '戰略級', 'priority': 3,
        'source': '老師明示 6/14 戰略級 + 6/16「黑K+籌碼漂亮」+ 6/18 突破加碼',
        'sector': '成熟製程',
        'note': '6/17 Stage 1 $139 × 1 張 + 6/18 13:00 Stage 2 $144 × 1 張 = 2 張 avg $141.5。6/23 stop trail $138→$160 (右上角規則 = 6/22 漲停收盤錨、收盤跌破才出、鎖利 ≥+13%)。原 $138.5 對右上角飆股太鬆。⚠️ 加碼時點 = 等回測近處新結構 + 外資 last-mile 翻買 (6/22 外資 -11,971 轉賣中、暫不加)。target $148-$152 已過、續抱看右上角延伸'
    },
    {
