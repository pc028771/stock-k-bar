...
def main():
    ...
    q = f"""SELECT trade_date, ticker, close, high, volume, ma5, ma20, ma20_slope,
                   bb_position, vol_ratio_20, main_force_1d
    ...
    cur_tk, hist = None, []
    # sitc (投信) net by (ticker, date)
    sitc = {}
    qs = f"""SELECT ticker, trade_date, sitc_net FROM institutional_investors
             WHERE trade_date BETWEEN ? AND ? AND ticker IN ({','.join('?'*len(tks))})"""
    for tk_, d_, n_ in conn.execute(qs, (START, END, *tks)):
        sitc[(tk_, d_)] = n_
    for r in conn.execute(q, (START, END, *tks)):
        d, tk, c, hi, vol, ma5, ma20, slope, bpos, vr, mf1 = r
        if tk != cur_tk:
            cur_tk, hist = tk, []
    ...
    for d in dates:
            ...
            for tk, row in rows.items():
                ...
                if row["slope"] > 0 and row["c"] > row["ma20"] \
                   and pv["c"] <= pv["ma5"] and row["c"] > row["ma5"]:
                    ...
                    if chg >= MAX_CHG:
                        continue
                    # Frank 籌碼鐵則 (raw/06 戰法一第一條 + raw/16 盤前剔除)
                    hist3 = chip_hist.get(tk, [])
                    # 剔除: 投信 or 主力 連 3 日賣超
                    if len(hist3) >= 3:
                        mf3 = [h[0] for h in hist3[-3:]]
                        st3 = [h[1] for h in hist3[-3:]]
                        if all(x is not None and x < 0 for x in mf3) or \
                           all(x is not None and x < 0 for x in st3):
                            continue
                    # 鐵則: 大戶(主力)買超 or 賣超收斂 (今日賣超 < 前日賣超)
                    mf_today = row.get("mf1")
                    mf_prev = pv.get("mf1")
                    if mf_today is not None:
                        selling_converge = (mf_today < 0 and mf_prev is not None
                                            and mf_today > mf_prev)
                        if not (mf_today > 0 or selling_converge):
                            continue
                    # Frank 戰法分數
                    score = 0
        ...
        for tk, row in rows.items():
            prev_day[tk] = row
            chip_hist.setdefault(tk, []).append((row.get("mf1"), row.get("sitc")))
            if len(chip_hist[tk]) > 5:
                chip_hist[tk] = chip_hist[tk][-5:]
    ...
    out = _REPO / "docs" / "frank" / "backtest" / "portfolio_sim_v3_chip.md"
    out.write_text("\n".join(lines))
    print(f"→ {out}")
