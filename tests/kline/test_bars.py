    ...
    with sqlite3.connect(db) as conn:
        conn.execute("""
            create table standard_daily_bar (
                ...
                ma5 real, ma10 real, ma20 real, ma60 real, ma240 real,
        ...
        conn.execute("""
            insert into standard_daily_bar values
                ('1101','2025-01-02',100,102,99,101,1000,100,100,100,100,100,1000,1.0,0,0,1),
                ('1101','2025-01-03',101,103,100,102,1100,100,100,100,100,100,1000,1.1,0,0,1)
        """)
        conn.commit()
...
def test_load_bars_filters_unusable(tmp_path: Path):
    ...
    with sqlite3.connect(db) as conn:
        ...
        conn.execute("""
            insert into standard_daily_bar values
                ('1101','2025-01-02',100,102,99,101,1000,100,100,100,100,100,1000,1.0,0,0,1),
                ('1101','2025-01-03',null,103,100,102,1100,100,100,100,100,100,1000,1.1,0,0,1)
        """)
        conn.commit()
    df = bars.load_bars(db_path=db, fill_from_backfill=False)
